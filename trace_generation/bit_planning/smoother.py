...
def proposed_path_smootherv2(old_path, new_path, env):
    ...
    for _ in range(K):
        ...
        if diff < 1e-5:
            return path
    return path
def proposed_path_smootherv2_probe(old_path, new_path, env):
    edge_info=[]
    edge_feas_info=[]
    K = int(np.ceil((np.linalg.norm(np.array(old_path) - np.array(new_path), axis=-1) / env.RRT_EPS).max()))
    path = deepcopy(old_path)
    for _ in range(K):
        ...
        for i, ns in enumerate(zip(path[1:-1], new_path[1:-1])):
            ...
            if dist < env.RRT_EPS:
                ...
                next_path[i] = env.interpolate(old_n, new_n, env.RRT_EPS / dist)
            efeas1,info,feas=env._edge_fp_probe(next_path[i-1], next_path[i])
            edge_info.append(info)
            edge_feas_info.append(feas)
            efeas2,info,feas=env._edge_fp_probe(next_path[i+1], next_path[i])
            edge_info.append(info)
            edge_feas_info.append(feas)
            if not (efeas1 and efeas2):
                next_path[i] = path[i]
            else:
                diff += np.linalg.norm(next_path[i]-new_n)
        path = next_path
        if diff < 1e-5:
            return path,edge_info,edge_feas_info
    return path,edge_info,edge_feas_info        
...
def model_smooth(model, free, collided, old_path, env, iter=5): 
    ...
    for iter_i in range(iter):
        ...
        old_path,info,feas_info = proposed_path_smootherv2_probe(old_path, new_path, env)
        if iter_i==0:
            edge_feas_info=feas_info
            edge_info=info
        else:
            edge_feas_info+=feas_info
            edge_info+=info
        #print(edge_feas_info)
    return old_path,edge_info,edge_feas_info
