...
class KukaEnv:
    ...
    def sample_n_points_probe(self, n, need_negative=False):
        ...
        for i in range(n):
            while True:
                sample = self.uniform_sample()
                #print("sampling")
                ans,link_info,link_feas_info=self._state_fp_probe(sample)
                sample_info.append([link_info])
                sample_feas_info.append([link_feas_info])
                if ans:
                    samples.append(sample)
                    break
                elif need_negative:
                    negative.append(sample)
        if not need_negative:
            return samples,sample_info,sample_feas_info
        else:
            return samples, negative,sample_info,sample_feas_info
    ...
    def step_probe(self, state, action=None, new_state=None, check_collision=True):
        '''
        Collision detection module
        '''
        #print("in step")
        ...
        no_collision,einfo,ecollinfo = self._edge_fp_probe(state, new_state)
        #print(no_collision)
        if no_collision and self.in_goal_region(new_state):
            done = True
    ...
    def _point_in_free_space(self, state):
        ...
        #print(len(p.getContactPoints(self.kukaId,self.kukaEndEffectorIndex)))
        p.performCollisionDetection()        
        if len(p.getContactPoints(self.kukaId)) == 0:
            ...
            #print("False")
            return False
    def _point_in_free_space_probe(self, state):
        t0 = time()
        if not self._valid_state(state):
            return False,[],[]
        #print(state)
        ...
        link_feas_info=[]
        link_info=[]
        for linkid in range(0,self.kukaEndEffectorIndex+1):
            numpon= len(p.getContactPoints(self.kukaId,linkIndexA=linkid)) 
            if numpon>0:
                link_feasible=0
                link_feas_info.append(0)
            else:
                link_feas_info.append(1)
            link_info.append(p.getLinkState(self.kukaId, linkid)[0])
        #print(link_feasible,len(p.getContactPoints(self.kukaId)))
        
        if len(p.getContactPoints(self.kukaId)) == 0:
            if link_feasible==0:
            #    #print("feas 0 error")
                print(link_feasible,link_feas_info,len(p.getContactPoints(self.kukaId)))
            self.collision_check_count += 1
            self.collision_time += time() - t0
            #print(state[0],",",state[1],",",state[2],",",state[3],",",state[4],",",state[5],",",state[6],",","True")
            return True,link_info,link_feas_info
        else:
            if link_feasible==1:
                #print("feas 0 error")
                print(link_feasible,link_feas_info,len(p.getContactPoints(self.kukaId)))
            ...
            self.collision_check_count += 1
            self.collision_time += time() - t0
            #print(state[0],",",state[1],",",state[2],",",state[3],",",state[4],",",state[5],",",state[6],",","False")
            return False,link_info,link_feas_info
    ...
    def _state_fp_probe(self, state):
        self.timer.start()
        free,link_info,link_feas_info = self._point_in_free_space_probe(state)
        self.timer.finish(Timer.VERTEX_CHECK)
        return free,link_info,link_feas_info
    ...
    def _edge_fp_probe(self, state, new_state):
        #print("edge_probe")
        edge_info=[]
        edge_feas_info=[]
        self.timer.start()
        ...
        if not self._valid_state(state) or not self._valid_state(new_state):
            self.timer.finish(Timer.EDGE_CHECK)
            return False,[],[]
        statefeas,info,feas_info=self._point_in_free_space_probe(state)
        edge_info.append(info)
        edge_feas_info.append(feas_info)
        newstatefeas,info,feas_info=self._point_in_free_space_probe(new_state)
        edge_info.append(info)
        edge_feas_info.append(feas_info)
        if not statefeas or not newstatefeas:
            self.timer.finish(Timer.EDGE_CHECK)
            collanswer=False
            #return False
        ...
        for k in range(0, K):
            c = state + k * 1. / K * disp
            statefeas,info,feas_info=self._point_in_free_space_probe(c)
            edge_info.append(info)
            edge_feas_info.append(feas_info)
            if not statefeas:
                #self.timer.finish(Timer.EDGE_CHECK)
        ...
        #print("Edge test",K,collanswer)  
        self.timer.finish(Timer.EDGE_CHECK)
        return collanswer,edge_info,edge_feas_info #True