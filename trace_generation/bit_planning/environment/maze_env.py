...
class MazeEnv:
    ...
    def sample_n_points_probe(self, n, need_negative=False):
        ...
        for i in range(n):
            while True:
                sample = self.uniform_sample()
                feas,einfo,ecollinfo=self._point_in_free_space_probe(sample)
                finfo.append([einfo])
                fcollinfo.append([ecollinfo])
                if (self.dim==2 and feas) or (self.dim==3 and self._stick_in_free_space(sample)):
                    samples.append(sample)
                    break
                elif need_negative:
                    negative.append(sample)
        if not need_negative:
            return samples,finfo,fcollinfo
        else:
            return samples, negative,finfo,fcollinfo
    ...
    def _point_in_free_space(self, state):
        ...
        ans=self.map[tuple(self._transform(state, self.width))] == 0
        #print(state[0],",",state[1],",",ans)
        #print(state,ans)
    ...
    def _point_in_free_space_probe(self, state):
        ...
        #print(state,ans,feas)
        #return self.map[tuple(self._transform(state, self.width))] == 0
        return ans,state,feas
    ...
    def _state_fp_probe(self, state):
        ...
        if state.size == 2:
            feas,einfo,ecollinfo=self._point_in_free_space_probe(state)
            return feas,einfo,ecollinfo
        elif state.size == 3:
            return self._stick_in_free_space(state)
    ...
    def _edge_fp_probe(self, state, new_state):
        ...
        if not self._valid_state(state) or not self._valid_state(new_state):
            return False,[],[]
        feas,info,coll=self._state_fp_probe(state)
        newfeas,newinfo,newcoll=self._state_fp_probe(new_state)
        feas_answer=True
        einfo.append(info)
        einfo.append(newinfo)
        ecollinfo.append(coll)
        ecollinfo.append(newcoll)
        if not feas or not newfeas:
            feas_answer=False
        #if not self._state_fp(state) or not self._state_fp(new_state):
        #    return False
