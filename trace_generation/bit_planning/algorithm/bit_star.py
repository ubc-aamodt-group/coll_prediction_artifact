...
class BITStar:
    ...
    def is_edge_free(self, edge):
        result,info,coll = self.env._edge_fp_probe(np.array(edge[0]), np.array(edge[1]))
        # self.T += self.env.k
        return result,info,coll
    ...
    def plan(self, pathLengthLimit, problemindex=2000,refine_time_budget=None, time_budget=None):
        ...
        while self.T < self.T_max and (time() - init_time < time_budget):
            if not self.vertex_queue and not self.edge_queue:
                ...
                self.prune(c_best)
                sample_temp,edge_info_full,edge_infocoll_full=self.sampling(c_best, self.batch_size, self.vertices)
            ...
            if best_edge_value < self.g_scores[self.goal]:
                ...
                actual_cost_of_edge,edgeinfo,edgeinfo_coll = self.actual_edge_cost(bestEdge[0], bestEdge[1])
                #print(edgeinfo)
